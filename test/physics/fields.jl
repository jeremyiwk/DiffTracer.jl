...
@testset "Derivatives" begin
    ...
    for name in names,
        ...
        @testset "Field: $(name), Multipole Order: $(ν)" begin
            ...
            @test ∇φr[1] ≈ -Exr(r) atol=1e-16
            @test ∇φr[2] ≈ -Eyr(r) atol=1e-16
            @test ∇φr[3] ≈ -Ezr(r) atol=1e-16
            @test ∇φi[1] ≈ -Exi(r) atol=1e-16
            @test ∇φi[2] ≈ -Eyi(r) atol=1e-16
            @test ∇φi[3] ≈ -Ezi(r) atol=1e-16

            ∇²φr = tr(ForwardDiff.hessian(φr, r))
            ∇²φi = tr(ForwardDiff.hessian(φi, r))

            @test ∇²φr ≈ 0.0 atol=1e-6
            @test ∇²φi ≈ 0.0 atol=1e-6
